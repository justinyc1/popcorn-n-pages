...
export const fetchTasteDive = async (searchQuery, mediaType) => {
    const tasteDiveResponse = await fetch(`https://tastedive.com/api/similar?q=${searchQuery}&type=${mediaType}&info=1&k=${TASTEDIVE_API_KEY}`);
    console.log("DEBUG 2");
...
export const fetchMovieData = async (mediaName) => {
    const tmdbResponse = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${TMDB_API_KEY}&query=${mediaName}`);
    const jsonData = await tmdbResponse.json();
    return jsonData;
}

export const fetchTVShowData = async (mediaName) => {
    const tmdbResponse = await fetch(`https://api.themoviedb.org/3/search/tv?api_key=${TMDB_API_KEY}&query=${mediaName}`);
    const jsonData = await tmdbResponse.json();
    return jsonData;
}

export const fetchBookData = async (mediaName) => {
    const bookResponse = await fetch(`https://www.googleapis.com/books/v1/volumes?q=intitle:${mediaName}&key=${GOOGLE_BOOKS_API_KEY}`);
